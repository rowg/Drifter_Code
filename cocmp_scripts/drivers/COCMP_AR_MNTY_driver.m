...
conf = MNTY_AR_conf;
logFile = 'MNTY_AR_conf.log';
fid = fopen(logFile,'w');

% time = datenum(2006,01,01,00,00,0.0001):1/24:datenum(2007,12,31,23,0,1);
time = datenum(2006,01,01,00,00,0.0001):1/24:datenum(2006,05,31,23,0,1);
    ...
    if procFlag == 2  ||  procFlag > 3
        ...
        try
            ...
            COCMPdriver_plot_hourly_totals(D,conf,'HourPlot.Type','OMA', ...
                                          ...
                                          'HourPlot.axisLims',[-122.75,conf.HourPlot.axisLims(2:4)], ...
                                          'HourPlot.Print',true);
        catch
            fprintf('COCMPdriver_plot_hourly_totals failed because:\n')
            res = lasterror;
        ...
        try
            ...
            COCMPdriver_plot_meanTUV(D,conf, ...
                          'HourPlot.axisLims',[-123.0,conf.HourPlot.axisLims(2:4)], ...
                          'HourPlot.Print',true);
        catch
...
END_TIME = now;
fprintf('COCMP_AR_MNTY_driver.m complete ...\n');
