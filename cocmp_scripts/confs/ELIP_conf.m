...
baseEllipseDir = fullfile(baseResultsDir,'Ellipticals');
...
conf.Radials.Sites = {'BLPA','GCSC','PCMT','PCSL','MTSL',...
                      'BLPA','GCSC','PCMT','PCSL','MTSL'};
conf.Radials.Types = {'ELTi','ELTi','ELTi','ELTi','ELTi',...
                      'ELTm','ELTm','ELTm','ELTm','ELTm'};
% conf.Radials.BaseDir = baseRadialDir;

% ********** Input the number of elliptical sites by hand ************
% NUMBER_OF_ELIPTICAL_SITES = 8;
% conf.Radials.BaseDir = repmat(baseRadialDir,length(conf.Radials.Types)-NUMBER_OF_ELIPTICAL_SITES,1);
% conf.Radials.BaseDir = cellstr(conf.Radials.BaseDir);
conf.Radials.BaseDir = baseEllipseDir;

% ADD the output path for the elliptical stuff
% for jj = 1:NUMBER_OF_ELIPTICAL_SITES
%     conf.Radials.BaseDir{end+1} = baseEllipseDir;
% end
...
% will then use unique ranges and unique bearings.
 conf.Radials.RangeLims = repmat([90,5,100],length(conf.Radials.Sites),1);
 conf.Radials.BearLims =  repmat([5,5,15],length(conf.Radials.Sites),1);
